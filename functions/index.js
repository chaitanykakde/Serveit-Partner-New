...
// Google Maps API key (set in Firebase environment)
 ...
 * @returns {Promise<Array>} Array of potential providers
 */
async function findProvidersWithGeoQuery(latitude, longitude, serviceName, radiusKm) {
  ...
  providersSnapshot.forEach(doc => {
    ...
    console.log(`Provider ${doc.id}: lat=${providerLat}, lng=${providerLng}, services=${JSON.stringify(providerData.services)}`);
    ...
    if (providerLng >= minLng && 
      ...
      console.log(`Provider ${doc.id}: REJECTED - outside bounds or service not available`);
    }
  });
